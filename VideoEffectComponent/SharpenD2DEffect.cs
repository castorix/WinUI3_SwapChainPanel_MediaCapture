...
{
    public sealed class SharpenD2DEffect : IBasicVideoEffect
    {
        [ComImport]
        ...
        private float? m_SharpnessSharpen = null;
        ...
        public unsafe void ProcessFrame(ProcessVideoFrameContext context)
        {
            ...
            if (m_SharpnessSharpen == null)
                m_SharpnessSharpen = SharpnessSharpen;
            ...
            hr = m_pD2DDeviceContextEffect.CreateEffect(CLSID_D2D1Sharpen, out pEffect);
            if (hr == HRESULT.S_OK)
            {
                pEffect.SetInput(0, inputBitmap);
                SetEffectFloat(pEffect, (uint)D2D1_SHARPEN_PROP.D2D1_SHARPEN_PROP_SHARPNESS, (float)m_SharpnessSharpen);
                SetEffectFloat(pEffect, (uint)D2D1_SHARPEN_PROP.D2D1_SHARPEN_PROP_THRESHOLD, 0.0f);
        ...
        public float SharpnessSharpen
        {
            get
            {
                object val;
                if (configuration != null && configuration.TryGetValue("SharpnessSharpen", out val))
                {
                    return (float)val;
                }
                return 5.0f;
