...
{
    public sealed class EdgeDetectionD2DEffect : IBasicVideoEffect
    {
        [ComImport]
        ...
        private float? m_StrengthEdgeDetection = null;
        ...
        public unsafe void ProcessFrame(ProcessVideoFrameContext context)
        {
            ...
            if (m_StrengthEdgeDetection == null)
                m_StrengthEdgeDetection = StrengthEdgeDetection;
            ...
            hr = m_pD2DDeviceContextEffect.CreateEffect(CLSID_D2D1EdgeDetection, out pEffect);
            if (hr == HRESULT.S_OK)
            {
                pEffect.SetInput(0, inputBitmap);
                SetEffectFloat(pEffect, (uint)D2D1_EDGEDETECTION_PROP.D2D1_EDGEDETECTION_PROP_STRENGTH, (float)m_StrengthEdgeDetection);
                SetEffectFloat(pEffect, (uint)D2D1_EDGEDETECTION_PROP.D2D1_EDGEDETECTION_PROP_BLUR_RADIUS, 0.0f);
                SetEffectInt(pEffect, (uint)D2D1_EDGEDETECTION_PROP.D2D1_EDGEDETECTION_PROP_MODE, (uint)D2D1_EDGEDETECTION_MODE.D2D1_EDGEDETECTION_MODE_SOBEL);
                SetEffectInt(pEffect, (uint)D2D1_EDGEDETECTION_PROP.D2D1_EDGEDETECTION_PROP_OVERLAY_EDGES, (uint)0);
                SetEffectInt(pEffect, (uint)D2D1_EDGEDETECTION_PROP.D2D1_EDGEDETECTION_PROP_ALPHA_MODE, (uint)D2D1_ALPHA_MODE.D2D1_ALPHA_MODE_PREMULTIPLIED);
        ...
        public float StrengthEdgeDetection
        {
            get
            {
                object val;
                if (configuration != null && configuration.TryGetValue("StrengthEdgeDetection", out val))
                {
                    return (float)val;
                }
                return 0.5f; 
