...
{
    public sealed class GaussianBlurD2DEffect : IBasicVideoEffect
    {
        [ComImport]
        ...
        private float? m_DeviationGaussianBlur = null;
        ...
        public unsafe void ProcessFrame(ProcessVideoFrameContext context)
        {
            ...
            if (m_DeviationGaussianBlur == null)
                m_DeviationGaussianBlur = DeviationGaussianBlur;
            ...
            hr = m_pD2DDeviceContextEffect.CreateEffect(CLSID_D2D1GaussianBlur, out pEffect);
            if (hr == HRESULT.S_OK)
            {
                pEffect.SetInput(0, inputBitmap);
                SetEffectFloat(pEffect, (uint)D2D1_GAUSSIANBLUR_PROP.D2D1_GAUSSIANBLUR_PROP_STANDARD_DEVIATION, (float)m_DeviationGaussianBlur);
        ...
        public float DeviationGaussianBlur
        {
            get
            {
                object val;
                if (configuration != null && configuration.TryGetValue("DeviationGaussianBlur", out val))
                {
                    return (float)val;
                }
                return 0.5f;
