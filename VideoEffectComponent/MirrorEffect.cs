...
namespace VideoEffectComponent
{
    public sealed class MirrorEffect : IBasicVideoEffect
    {
        ...
        private bool? m_IsVertical = null;
        ...
        public unsafe void ProcessFrame(ProcessVideoFrameContext context)
        {
            ...
            if (m_IsVertical == null)
                m_IsVertical = IsVertical;

            Matrix3x2F mirrorTransform;
            if ((bool)m_IsVertical)
                mirrorTransform = Matrix3x2F.Scale(1.0f, -1.0f) * Matrix3x2F.Translation(0, nHeight);
            else
                mirrorTransform = Matrix3x2F.Scale(-1.0f, 1.0f) * Matrix3x2F.Translation(nWidth, 0);
            ...
            var scaleMatrix = Matrix3x2F.Scale(nScaleX, nScaleY);
            var resultMatrix = scaleMatrix * mirrorTransform;

            m_pD2DDeviceContextEffect.SetTransform(resultMatrix);
            ...
            m_pD2DDeviceContextEffect.EndDraw(out ulong tag1, out ulong tag2);
        ...
        public bool IsVertical
        {
            ...
            {
                object val;
                if (configuration != null && configuration.TryGetValue("IsVertical", out val))
                {
                    return (bool)val;
                }
                return false;
