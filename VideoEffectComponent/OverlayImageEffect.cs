...
{
    public sealed class OverlayImageEffect : IBasicVideoEffect
    {
        [ComImport]
        ...
        private SoftwareBitmap m_OverlayImage = null;
        ...
        void IBasicVideoEffect.Close(MediaEffectClosedReason reason)
        {
            ...
            SafeRelease(ref m_pD2D1Bitmap1);            
        }
        ...
        ID2D1Bitmap1 m_pD2D1Bitmap1 = null;
        ...
        public unsafe void ProcessFrame(ProcessVideoFrameContext context)
        {
            ...
            if (m_OverlayImage == null)
            {
                ...
                m_pD2D1Bitmap1 = ConvertSoftwareBitmaptoD2DBitmap(m_OverlayImage, m_pD2DDeviceContextEffect);
            }
            ...
            float nDPIXInput = 96.0f;
            float nDPIYInput = 96.0f;
            //m_pD2DDeviceContextEffect.GetDpi(out float nDPIXTarget, out float nDPIYTarget);
            uint nDPI = GetDpiForWindow(FindWindow("Shell_TrayWnd", null));
            float nScaleX = nDPI / nDPIXInput;
            float nScaleY = nDPI / nDPIYInput;          

            m_pD2DDeviceContextEffect.SetTransform(Matrix3x2F.Scale(nScaleX, nScaleY));
            ...
            if (m_pD2D1Bitmap1 != null)
            {
                ...
                m_pD2DDeviceContextEffect.DrawBitmap(m_pD2D1Bitmap1, ref destRectOverlay, 1.0f, D2D1_INTERPOLATION_MODE.D2D1_INTERPOLATION_MODE_LINEAR, ref sourceRectOverlay);
            }

            m_pD2DDeviceContextEffect.SetTransform(Matrix3x2F.Identity());
        ...
        public SoftwareBitmap OverlayImage
        {
            get
            {
                object val;
                if (configuration != null && configuration.TryGetValue("OverlayImage", out val))
                {
                    // To avoid System.InvalidCastException (different Context)
                    var objRef = WinRT.MarshalInspectable<object>.FromManaged(val);
                    if (objRef != IntPtr.Zero)
                    {
                        return WinRT.MarshalInterface<SoftwareBitmap>.FromAbi(objRef);
                    }
                }
                return null;
            }
        ...
        private unsafe ID2D1Bitmap1 ConvertSoftwareBitmaptoD2DBitmap(SoftwareBitmap softwareBitmap, ID2D1DeviceContext pD2DdeviceContext)
        {
            ID2D1Bitmap1 pD2D1Bitmap1 = null;
            using (BitmapBuffer buffer = softwareBitmap.LockBuffer(BitmapBufferAccessMode.Read))
            {
                using (var reference = buffer.CreateReference())
                {
                    byte* pByteArray;
                    reference.As<IMemoryBufferByteAccess>().GetBuffer(out pByteArray, out uint inputCapacity);
                    BitmapPlaneDescription bpDesc = buffer.GetPlaneDescription(0);
                    uint nDPI = GetDpiForWindow(FindWindow("Shell_TrayWnd", null));
                    var props = new D2D1_BITMAP_PROPERTIES1()
                    {
                        pixelFormat = new D2D1_PIXEL_FORMAT()
                        ...
                        //dpiX = (float)softwareBitmap.DpiX,
                        //dpiY = (float)softwareBitmap.DpiY,
                        dpiX = nDPI,
                        dpiY = nDPI,
                    };
                    HRESULT hr = pD2DdeviceContext.CreateBitmap(new D2D1_SIZE_U((uint)softwareBitmap.PixelWidth, (uint)softwareBitmap.PixelHeight),
                        new nint(pByteArray + bpDesc.StartIndex), (uint)bpDesc.Stride, ref props, out pD2D1Bitmap1);
                }
            }
            return pD2D1Bitmap1;
        }      
