...
{
    public sealed class VignetteD2DEffect : IBasicVideoEffect
    {
        [ComImport]
        ...
        private float? m_VignetteStrength = null;
        private float? m_VignetteTransitionSize = null;
        private Windows.UI.Color m_VignetteColor = Microsoft.UI.Colors.Transparent;
        ...
        public unsafe void ProcessFrame(ProcessVideoFrameContext context)
        {
            ...
            if (m_VignetteStrength == null)
                m_VignetteStrength = VignetteStrength;
            if (m_VignetteTransitionSize == null)
                m_VignetteTransitionSize = VignetteTransitionSize;
            if (m_VignetteColor == Microsoft.UI.Colors.Transparent)
                m_VignetteColor = VignetteColor;
            ...
            hr = m_pD2DDeviceContextEffect.CreateEffect(CLSID_D2D1Vignette, out pEffect);
            if (hr == HRESULT.S_OK)
            {
                pEffect.SetInput(0, inputBitmap);
                float[] aFloatArray = { (float)((float)m_VignetteColor.R / 255.0f), (float)((float)m_VignetteColor.G / 255.0f), (float)((float)m_VignetteColor.B / 255.0f), 1.0f };
                SetEffectFloatArray(pEffect, (uint)D2D1_VIGNETTE_PROP.D2D1_VIGNETTE_PROP_COLOR, aFloatArray);
                SetEffectFloat(pEffect, (uint)D2D1_VIGNETTE_PROP.D2D1_VIGNETTE_PROP_TRANSITION_SIZE, (float)m_VignetteTransitionSize);
                SetEffectFloat(pEffect, (uint)D2D1_VIGNETTE_PROP.D2D1_VIGNETTE_PROP_STRENGTH, (float)m_VignetteStrength);
        ...
        public float VignetteStrength
        {
            ...
            {
                object val;
                if (configuration != null && configuration.TryGetValue("VignetteStrength", out val))
                {
                    return (float)val;
                }
                return 0.5f;
        ...
        public float VignetteTransitionSize
        {
            get
            {
                object val;
                if (configuration != null && configuration.TryGetValue("VignetteTransitionSize", out val))
                {
                    return (float)val;
                }
                return 0.5f;
        ...
        public Windows.UI.Color VignetteColor
        {
            get
            {
                object val;
                if (configuration != null && configuration.TryGetValue("VignetteColor", out val))
                {
                    ...
                    {
                        return WinRT.MarshalInterface<Windows.UI.Color>.FromAbi(objRef);
                    }
                }
