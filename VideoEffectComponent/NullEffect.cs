...
{
    public sealed class NullEffect : IBasicVideoEffect
    {
        [ComImport]
        ...
        private ID3D11Device GetD3D11Device(IDirect3DDevice pDevice)
        {
            ...
            try
            {
                var dia = pDevice.As<IDirect3DDxgiInterfaceAccess>();
                Guid guid = typeof(ID3D11Device).GUID;
                HRESULT hr = dia.GetInterface(guid, out pDevicePtr);
                if (hr == HRESULT.S_OK && pDevicePtr != IntPtr.Zero)
                {
                    pD3D11Device = (ID3D11Device)Marshal.GetObjectForIUnknown(pDevicePtr);
                }
            }
            finally
            {
        ...
        private IDXGISurface GetDXGISurface(IDirect3DSurface pSurface)
        {
            ...
            try
            {
                var dia = pSurface.As<IDirect3DDxgiInterfaceAccess>();
                Guid guid = typeof(IDXGISurface).GUID;
                HRESULT hr = dia.GetInterface(guid, out pSurfacePtr);
                if (hr == HRESULT.S_OK && pSurfacePtr != IntPtr.Zero)
                {
                    pDXGISurface = (IDXGISurface)Marshal.GetObjectForIUnknown(pSurfacePtr);
                }
            }
            finally
            {
                if (pSurfacePtr != IntPtr.Zero)
                {
